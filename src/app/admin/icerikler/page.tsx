...
export const metadata = { title: "Admin Icerikler" };
...
export default async function AdminContentPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-display text-2xl text-snow-100">Icerik Yonetimi</h1>
        <p className="mt-1 text-sm text-snow-300/60">Harita ve egitim iceriklerini ekle.</p>
      </div>
      <ContentForm />
      <div className="grid gap-4 sm:grid-cols-2">
        {items.map((item) => (
          <Card key={item.id}>
            <p className="text-xs uppercase tracking-[0.2em] text-frost-ice/80">{item.type === "MAP" ? "Harita" : "Egitim"}</p>
            ...
            {item.url && <a href={item.url} target="_blank" rel="noreferrer" className="mt-3 inline-block text-sm text-frost-ice hover:text-frost-teal">Baglantiyi ac -&gt;</a>}
          </Card>
        ))}
      </div>
    </div>
